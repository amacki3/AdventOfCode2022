...
class Monkey:
    ...
    def throw(self):
        ...
        for item in self.items:
            if test(item) == 0:
                self.nextMonkey[0].items.append(item % self.modulusFactor)
                #print("Throwing item with worry level {} TRUE".format(item))
            else:
                self.nextMonkey[1].items.append(item % self.modulusFactor)
...
#Lets keep numbers manageable - we can modulus them by the total of all the monkey tests!
modulusFactor = monkey[0].testDivisor
for m in monkey[1:]:
    modulusFactor *= m.testDivisor

Monkey.modulusFactor = modulusFactor
